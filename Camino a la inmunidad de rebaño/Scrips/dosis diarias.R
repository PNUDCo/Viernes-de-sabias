...
EDAD<- read_excel("vacr.xlsx")
EDAD$edad=factor(EDAD$edad, levels=c("80 y más","70 A 79","60 a 69","50 a 59","40 a 49"))

EDAD$Fecha<- as.Date(EDAD$Fecha)


lm<- ggplot(EDAD, aes(x = fecha, y = valor,label=sprintf("%0.2f", round(valor, digits = 2))))+ 
  geom_line(aes(color= valor), size = 1)+
  ...
  labs(x= "Mes",
       y= "Número de dosis",
       color= "Cantidad",
       title= "Número de dosis aplicadas por día ",
       subtitle= "Total nacional. Marzo - julio 2021. Corte 7 de julio",
       caption= "Fuente: PNUD con base Instituto Nacional de Salud ")+
  theme_bw() +
