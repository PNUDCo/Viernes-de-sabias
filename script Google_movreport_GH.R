...
###Renombra variables
movcolTB2<-rename(movcolTB, Lugar = cate)
#limpia el ruido creado por los missings
is.na(movcolTB2)
##verifica los niveles que tiene la variable Lugar
levels(movcolTB$Lugar)
##Determina cuales son los niveles que quiere que tenga la variable Lugar
movcolTB2$Lugar=factor(movcolTB2$Lugar, levels=c("retail_and_recreation_percent_change_from_baseline", "parks_percent_change_from_baseline","transit_stations_percent_change_from_baseline","workplaces_percent_change_from_baseline","residential_percent_change_from_baseline","grocery_and_pharmacy_percent_change_from_baseline"))
levels(movcolTB$Lugar)
...
movcolTB2$Lugar <- gsub("retail_and_recreation_percent_change_from_baseline", "Tiendas de ocio", movcolTB2$Lugar)
movcolTB2$Lugar <- gsub("parks_percent_change_from_baseline", "Parques", movcolTB2$Lugar)
movcolTB2$Lugar <- gsub("transit_stations_percent_change_from_baseline", " Estaciones de transporte  ", movcolTB2$Lugar)
movcolTB2$Lugar <- gsub("workplaces_percent_change_from_baseline", "Lugares de trabajo ", movcolTB2$Lugar)
movcolTB2$Lugar <- gsub("residential_percent_change_from_baseline", "Zonas residenciales", movcolTB2$Lugar)
movcolTB2$Lugar<- gsub("grocery_and_pharmacy_percent_change_from_baseline", "Supermercados y farmacias", movcolTB2$Lugar)
...
ggplot(data=ll,aes(x = date,y = value ,col = Lugar)) +
  geom_smooth()+
